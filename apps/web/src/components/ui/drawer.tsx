...
function DrawerOverlay({
  ...
  return (
    <DrawerPrimitive.Overlay
      ...
      className={cn(
        // Critical: pointer-events-auto + touch-none ensures the backdrop
        // absorbs taps/swipes that would otherwise reach the map underneath.
        // Tailwind's data-[state=...] variants match the data-state attribute
        // Radix Dialog sets on the overlay — `data-open:` (no brackets) is
        // not a real variant and wouldn't fire.
        "fixed inset-0 z-50 touch-none bg-black/40 pointer-events-auto supports-backdrop-filter:backdrop-blur-sm data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=closed]:animate-out data-[state=closed]:fade-out-0",
        className
      )}
      {...props}
    />
